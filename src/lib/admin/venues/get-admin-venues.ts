...
function mapVenue(
  ...
  return {
    ...
    categoryNameFr: row.category?.name_fr ?? "Sans catégorie",

    categoryNameEn: row.category?.name_en ?? "Uncategorized",
  };
}

/* =========================================================
   Mapping ancienne présentation d'usage
   ========================================================= */

function mapUseSummary(
  row: UsePresentationRow,
  images: UseImageRow[],
): AdminVenueUsePresentationSummary {
  const cover = [...images].sort(
    (a, b) =>
...
export async function getAdminVenues() {
  ...
  const [venues, images, links, uses] = await Promise.all([
    ...
    supabase
      ...
      .select("venue_id,setup_type_id,is_active")
      .eq("is_active", true),
  ...
  if (venues.error || images.error || links.error || uses.error) {
    console.error(
      "[admin-venues] Load failed:",
  ...
  /* ---------------------------------------------------------
     Nombre d'anciens usages
     --------------------------------------------------------- */

  const useCounts = new Map<string, number>();

  for (const use of (uses.data ?? []) as unknown as Array<{
    venue_id: string;
  }>) {
  ...
  return {
    ...
    venues: ((venues.data ?? []) as unknown as VenueRow[]).map((row) =>
      mapVenue(
        row,
        imagesByVenue.get(row.id) ?? [],
        setupCounts.get(row.id) ?? 0,
        useCounts.get(row.id) ?? 0,
      ),
    ),
  };
}
...
export async function getAdminVenue(id: string) {
  ...
  const [venue, images, links, uses, useImages] = await Promise.all([
    ...
    supabase
      .from("venue_setup_links")
      .select("setup_type_id,is_active")
      .eq("venue_id", id)
      .eq("is_active", true),
  ...
  if (
    ...
    console.error(
      "[admin-venues] Detail load failed:",
      venue.error?.message ??
        images.error?.message ??
        links.error?.message ??
        uses.error?.message ??
        useImages.error?.message,
    );
  ...
  /* ---------------------------------------------------------
     Images des anciennes présentations
     --------------------------------------------------------- */
  ...
  const usePresentations = (
    (uses.data ?? []) as unknown as UsePresentationRow[]
  ).map((use) => mapUseSummary(use, imagesByUse.get(use.id) ?? []));
  ...
  const detail: AdminVenueDetail = {
    ...mapVenue(
      ...
      usePresentations.length,
    ),

    surfaceM2: row.surface_m2 !== null ? String(row.surface_m2) : "",
    ...
    ).map((link) => link.setup_type_id),

    /*
     * Catégorie automatiquement présélectionnée
...
export async function getAdminVenueCategories() {
  const supabase = (await getSupabaseServerClient()).schema("site");
  ...
  if (error) {
    console.error("[admin-venues] Categories load failed:", error.message);
  ...
  return {
    ...
    categories: ((data ?? []) as unknown as CategoryRow[]).map(
      (category): AdminVenueCategory => ({
        id: category.id,
...
/* =========================================================
   ANCIEN SYSTÈME DES TYPES D'USAGE
   Conservé temporairement mais plus utilisé dans le
   workflow principal du backoffice.
   ========================================================= */
...
export async function getAdminVenueUseTypes() {
  ...
  return {
    ...
    useTypes: (
      (types.data ?? []) as unknown as Array<{
        id: string;
        code: string;
        name_fr: string;
        name_en: string;
...
export async function getAdminVenueUseType(id: string) {
  ...
  if (typeResult.error || uses.error) {
    console.error(
      "[admin-venues] Use type detail load failed:",
      typeResult.error?.message ?? uses.error?.message,
    );
...
/* =========================================================
   Ancienne présentation dédiée
   ========================================================= */

export async function getAdminVenueUsePresentation(
  venueId: string,
  ...
  if (presentation.error || images.error) {
    console.error(
      "[admin-venues] Use presentation load failed:",
      presentation.error?.message ?? images.error?.message,
    );
...
export async function getAdminVenueSetups() {
  ...
  for (const link of (links.data ?? []) as unknown as Array<{
    setup_type_id: string;
  }>) {
