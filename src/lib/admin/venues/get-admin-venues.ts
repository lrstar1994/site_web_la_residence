...
import type { AdminVenue, AdminVenueDetail, AdminVenueSetup } from "@/lib/admin/venues/admin-venue-types";
...
function mapVenue(row: VenueRow, images: ImageRow[], setupCount: number): AdminVenue {
...
export async function getAdminVenues() {
  ...
  const [venues, images, links] = await Promise.all([
    supabase
      .from("venues")
      .select(
  ...
  return {
    ...
    venues: ((venues.data ?? []) as unknown as VenueRow[]).map((row) =>
      mapVenue(row, imagesByVenue.get(row.id) ?? [], setupCounts.get(row.id) ?? 0),
    ),
  };
}
...
export async function getAdminVenue(id: string) {
  ...
  const [venue, images, links] = await Promise.all([
    supabase
      .from("venues")
      .select(
        "id,code,name,location_fr,location_en,short_description_fr,short_description_en,capacity,surface_m2,sort_order,is_active,updated_at",
  ...
  if (venue.error || images.error || links.error) {
    console.error("[admin-venues] Detail load failed:", venue.error?.message ?? images.error?.message ?? links.error?.message);
  ...
  const detail: AdminVenueDetail = {
    ...mapVenue(row, (images.data ?? []) as unknown as ImageRow[], ((links.data ?? []) as unknown as LinkRow[]).length),
