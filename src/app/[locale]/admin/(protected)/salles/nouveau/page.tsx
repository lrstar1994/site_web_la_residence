...
export const metadata: Metadata = {
  title: "Nouvelle salle | Administration",
  robots: {
    index: false,
    follow: false,
    noarchive: true,
...
export default async function NewVenuePage({
  ...
  if (locale !== "fr") {
    redirect("/fr/admin/salles/nouveau");
  }

  await requireAdmin(locale as Locale);

  const [setups, categories] = await Promise.all([
    getAdminVenueSetups(),
    getAdminVenueCategories(),
  ]);
  ...
  return (
    <AdminVenueForm
      mode="create"
      setups={setups.ok ? setups.setups : []}
      categories={
        categories.ok ? categories.categories : []
      }
    />
  );
}